...
//        $PaymentId = $json["Data"]["PaymentId"];
        ...
        try {
            ...
            $this->saveOrderDetails($order, $PaymentId);
            DB::commit();
...
//            'status' => Order::PAID,
    ...
    private function saveOrderDetails(Order $order, $PaymentId)
    {
        if (session()->has('basket')){
            foreach (session('basket') as $item){
                $product = Product::where('id', $item['product_id'])->where('qty', '>=', $item['quantity'])->first();
                if ($product){
                    OrderDetails::create([
                        'order_id' => $order->id,
